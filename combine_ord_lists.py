...
def combine_ord_lists(lst1, lst2, key=lambda x: x): # lst1 and lst2 are ordered
    """Basic recursive combining ordered lists function"""

    if not lst1 or not lst2:
        return lst1 or lst2
    elif key(lst1[0]) <= key(lst2[0]):
        return lst1[:1] + combine_ord_lists(lst1[1:], lst2, key)
    else:
        return lst2[:1] + combine_ord_lists(lst1, lst2[1:], key)

@trampoline
def combine_ord_lists_tr(lst1, lst2, key=lambda x: x): # lst1 and lst2 are ordered
